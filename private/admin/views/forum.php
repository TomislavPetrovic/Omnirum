...
<?php
	...
	//dio koda koji obrađuje zahtjev admina za brisanje teme
	if(isset($_GET['delete_topic_id']) && (strlen($_GET['delete_topic_id']) < 11)){
		if($topic->deleteTopic($_GET['delete_topic_id']) === true){
			self::addGlobalMessage('Tema je uspješno izbrisana!');
		} else {
			self::addGlobalMessage('Tema nije izbrisana!');
		}
	} 
...
?>
		...
		<td style="text-align: center;"><a href="forum.php?delete_topic_id=<?php echo $item['id']; ?>">Izbriši</a></td>
	</tr>
